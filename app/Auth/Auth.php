...
class Auth
{
    ...
    public static function create($email, $username, $password, $info = [])
    {
        ...
        try {
            $userId = $auth->register($email, $username, $password, function ($selector, $token) use ($email, $username) {
                //check for first entry and make it super admin role
                if (User::count() === 1) {
                    $user = User::first();
                    $user->verified = 1;
                    $user->roles_mask = Role::SUPER_ADMIN;
                    $user->save();
                } else {
    ...
    public static function createApi($email, $username, $password, $info = [])
    {
        $auth = self::$auth;
        try {
            $userId = $auth->register($email, $username, $password, function ($selector, $token) use ($email, $username) {
    ...
    {
        if ($role === 'super_admin') {
            $srole = Role::SUPER_ADMIN;
        } elseif ($role === 'admin') {
            $srole = Role::ADMIN;
        } elseif ($role === 'user') {
            $srole = Role::SUBSCRIBER;
        } else {
            $srole = '';
        }
        $auth = self::$auth;
        if ($auth->hasRole($srole)) {
            return true;
        }
        return false;
    }
    ...
    public static function hasAnyRole($roles = [])
    {
        ...
        foreach ($roles as $key => $role) {
            if ($role === 'super_admin') {
                $srole[$key] = Role::SUPER_ADMIN;
            } elseif ($role === 'admin') {
                $srole[$key] = Role::ADMIN;
            } elseif ($role === 'user') {
                $srole[$key] = Role::SUBSCRIBER;
            } else {
                $srole = '';
            }
        }
        $froles = implode(',', $srole);
        if ($auth->hasAnyRole($froles)) {
            return true;
        }
        return false;
    }
